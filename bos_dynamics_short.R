...
for (i in 1:length(uniquetime)){
  df_treat = filter(df, time == uniquetime[i])
...
rm(df_treat, df_treat_1, df_treat_2, ordereffect)
...
sum_pair = matrix(NA, nrow = 2, ncol = 2)
...
sum_pair2 = matrix(NA, nrow = 1, ncol = 2)
rownames(sum_pair2) = c('C-D')
